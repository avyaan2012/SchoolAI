...
let selectedAnswers = {};
let currentResultsAttempt = null;
let currentQuestionAIContext = null;
let questionAIConversation = [];
let questionAISending = false;
let questionAIRequestVersion = 0;
...
function showPage(pageId) {
  if (pageId !== "results") closeQuestionAI();
...
function renderResults(attempt) {
  ...
  answerReview.innerHTML = attempt.results.map((result, index) => `
    <div class="result-card question-result-card ${result.isCorrect ? "correct" : "incorrect"}">
      <div class="question-result-heading">
        <h3>${escapeHtml(result.question.questionText)}</h3>
        <div class="question-options">
          <button
            type="button"
            class="question-options-button"
            data-question-menu-button="${index}"
            aria-label="Question options"
            aria-haspopup="menu"
            aria-expanded="false"
          >⋮</button>
          <div
            class="question-options-menu hidden"
            data-question-menu="${index}"
            role="menu"
          >
            <button
              type="button"
              class="ask-ai-question-button"
              data-ask-ai-index="${index}"
              role="menuitem"
            >
  ...
  answerReview.querySelectorAll("[data-question-menu-button]").forEach(button => {
    button.addEventListener("click", event => {
      event.stopPropagation();
      toggleQuestionOptionsMenu(Number(button.dataset.questionMenuButton), button);
    });
  });

  answerReview.querySelectorAll("[data-ask-ai-index]").forEach(button => {
    button.addEventListener("click", event => {
      event.stopPropagation();
      openQuestionAI(Number(button.dataset.askAiIndex));
    });
  });
...
function toggleQuestionOptionsMenu(index, button) {
  const menu = document.querySelector(`[data-question-menu="${index}"]`);
  if (!menu) return;

  const shouldOpen = menu.classList.contains("hidden");
  closeQuestionOptionsMenus();
...
function closeQuestionOptionsMenus() {
  document.querySelectorAll(".question-options-menu").forEach(menu => {
...
function openQuestionAI(resultIndex) {
  closeQuestionOptionsMenus();

  const attempt = currentResultsAttempt;
  const result = attempt?.results?.[resultIndex];
  if (!attempt || !result) return;

  questionAIRequestVersion += 1;
  currentQuestionAIContext = {
    attempt,
    result,
    resultIndex
  };
  questionAIConversation = [];
  questionAISending = false;

  const overlay = document.getElementById("questionAIOverlay");
  const title = document.getElementById("questionAIQuestion");
  const topic = document.getElementById("questionAITopic");
  const studentAnswer = document.getElementById("questionAIStudentAnswer");
  const correctAnswer = document.getElementById("questionAICorrectAnswer");
  const status = document.getElementById("questionAIStatus");
  const input = document.getElementById("questionAIInput");

  if (!overlay || !title || !studentAnswer || !correctAnswer) return;

  title.textContent = result.question.questionText;
  topic.textContent =
    result.question.weakTopic ||
    attempt.quiz.topic ||
    "Quiz question";
  studentAnswer.textContent = result.answer || "No answer";
  correctAnswer.textContent = result.question.correctAnswer || "Not provided";
  status.textContent = "";
  input.value = "";

  overlay.classList.remove("hidden");
  document.body.classList.add("ai-panel-open");
  renderQuestionAIMessages();

  const starterMessage = result.isCorrect
    ? "Help me understand why the correct answer is correct, using my class resources."
    : "Explain why my answer was wrong and why the correct answer is right. Use my class resources and point me to the exact material.";

  sendQuestionAIMessage(starterMessage, { showUserMessage: false });
  window.setTimeout(() => input.focus(), 100);
}

function closeQuestionAI() {
  questionAIRequestVersion += 1;
  const overlay = document.getElementById("questionAIOverlay");
...
function handleQuestionAIKeydown(event) {
  if (event.key === "Enter" && !event.shiftKey) {
    event.preventDefault();
    submitQuestionAIForm(event);
  }
}

function submitQuestionAIForm(event) {
  event?.preventDefault();
  const input = document.getElementById("questionAIInput");
  if (!input) return;
  ...
  input.value = "";
  sendQuestionAIMessage(message, { showUserMessage: true });
}

function sendQuestionAISuggestion(prompt) {
  if (!prompt) return;
  sendQuestionAIMessage(prompt, { showUserMessage: true });
}

async function sendQuestionAIMessage(message, { showUserMessage = true } = {}) {
  if (!currentQuestionAIContext || questionAISending || !message.trim()) return;

  if (showUserMessage) {
    questionAIConversation.push({
      role: "user",
      content: message.trim()
    });
  }

  questionAISending = true;
  const requestVersion = questionAIRequestVersion;
  setQuestionAIStatus("");
  renderQuestionAIMessages();

  const { attempt, result } = currentQuestionAIContext;
  const question = result.question;

  try {
    const response = await fetch(getQuestionHelpEndpoint(), {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
        question: {
          ...
          slideRange: question.slideRange
        },
        message: message.trim(),
        conversationHistory: questionAIConversation.slice(-10),
        materials: getQuestionAIMaterials(attempt, question)
      })
    });
    ...
    if (!response.ok) {
      throw new Error(
        data.error ||
        data.message ||
        `Question-help endpoint returned ${response.status}`
      );
    }

    if (requestVersion !== questionAIRequestVersion) return;

    questionAIConversation.push({
      role: "assistant",
      content: String(data.answer || "I could not create an explanation."),
      mode: data.mode === "web" ? "web" : "school",
    ...
    if (requestVersion !== questionAIRequestVersion) return;
    setQuestionAIStatus(
      `AI help failed: ${error.message}. Make sure /api/question-help is deployed and GEMINI_API_KEY is set in Vercel.`,
      "error"
    );
  } finally {
    if (requestVersion !== questionAIRequestVersion) return;
...
function getQuestionAIMaterials(attempt, question) {
  const cls = state.classes.find(item => item.id === attempt.quiz.classId);
  const materials = (
    attempt.quiz.relevantMaterials ||
    cls?.materials ||
    []
  ).filter(material => material.type !== "Sync Error");
  ...
  return [...materials]
    .sort((first, second) => {
      const firstPriority =
        ...
        first.title === question.sourceMaterialTitle
          ? 1
          : 0;
      const secondPriority =
        second.id === question.sourceMaterialId ||
        second.title === question.sourceMaterialTitle
          ? 1
          : 0;
    ...
    .map(material => ({
      ...
      location:
        material.id === question.sourceMaterialId
          ? question.slideRange || ""
          : "",
      text: truncate(material.text, 6000)
    }));
}

function getQuestionHelpEndpoint() {
  if (
    typeof AI_QUESTION_HELP_URL !== "undefined" &&
    String(AI_QUESTION_HELP_URL || "").trim()
  ) {
    return String(AI_QUESTION_HELP_URL).trim();
  }

  return "/api/question-help";
}

function renderQuestionAIMessages() {
  const messagesBox = document.getElementById("questionAIMessages");
  const suggestionsBox = document.getElementById("questionAISuggestions");
  const sendButton = document.getElementById("questionAISendButton");
  const input = document.getElementById("questionAIInput");

  if (!messagesBox || !suggestionsBox) return;

  if (questionAIConversation.length === 0 && !questionAISending) {
    messagesBox.innerHTML = `
      <div class="ai-empty-message">
        Ask why an answer is correct, request a simpler explanation, or ask where the topic appears in your school materials.
      </div>
    `;
  } else {
    messagesBox.innerHTML = questionAIConversation.map(message => {
      const roleClass =
        message.role === "user"
      ...
      const sourceHtml =
        message.role === "assistant" && Array.isArray(message.sources) && message.sources.length
          ? `
            <div class="question-ai-sources">
              <strong>Resources used</strong>
              ${message.sources.map(source => {
                const label = `${source.name || "Resource"}${source.location ? ` — ${source.location}` : ""}`;
      ...
      const modeHtml =
        message.role === "assistant" && message.mode === "school"
          ? `<span class="question-ai-mode school">Based on school resources</span>`
          : message.role === "assistant" && message.mode === "web"
            ? `<span class="question-ai-mode web">School resources were insufficient — trusted web used</span>`
            : "";

      return `
        <article class="${roleClass}">
          <div class="question-ai-message-label">${message.role === "user" ? "You" : "AI"}</div>
          <div class="question-ai-message-text">${escapeHtml(message.content)}</div>
          ${modeHtml}
          ${sourceHtml}
        </article>
      `;
  ...
  if (questionAISending) {
    messagesBox.insertAdjacentHTML(
      "beforeend",
      `
        <article class="question-ai-message assistant">
          <div class="question-ai-message-label">AI</div>
          <div class="question-ai-loading" aria-label="AI is responding">
  ...
  const lastAssistant = [...questionAIConversation]
    .reverse()
    .find(message => message.role === "assistant");

  const suggestions =
    lastAssistant?.followUpSuggestions?.length
      ? lastAssistant.followUpSuggestions
      : [
          "Explain this more simply.",
          "Give me a similar practice question.",
          "Where is this in my class resources?"
        ];
  ...
  suggestionsBox.innerHTML = suggestions
    ...
    .map(suggestion => `
      <button
        type="button"
        class="question-ai-suggestion"
        data-question-ai-suggestion="${escapeAttribute(suggestion)}"
        ${questionAISending ? "disabled" : ""}
      >${escapeHtml(suggestion)}</button>
    `)
    .join("");

  suggestionsBox
    .querySelectorAll("[data-question-ai-suggestion]")
    .forEach(button => {
      button.addEventListener("click", () => {
        sendQuestionAISuggestion(button.dataset.questionAiSuggestion);
      });
    });

  if (sendButton) sendButton.disabled = questionAISending;
  if (input) input.disabled = questionAISending;

  messagesBox.scrollTop = messagesBox.scrollHeight;
}

function setQuestionAIStatus(message, type = "") {
  const status = document.getElementById("questionAIStatus");
  if (!status) return;

  status.className = `question-ai-status ${type}`.trim();
...
document.addEventListener("click", event => {
  if (!event.target.closest(".question-options")) {
    closeQuestionOptionsMenus();
  }
});
